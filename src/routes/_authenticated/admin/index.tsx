  ...
  return (
    <div className="space-y-8">
      ...
      <section className="flex flex-wrap gap-2">
        <QA to="/admin/bookings" search={{ status: "pending_approval" }}>
          Review pending
        </QA>
        <QA to="/admin/bookings" search={{ status: "awaiting_payment" }}>
          Chase payments
        </QA>
      ...
      <section>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Stat
            ...
            label="Today"
            value={money(rev?.today ?? 0)}
            delta={delta(rev?.today, rev?.prevDay)}
          />
          <Stat
            icon={<DollarSign className="h-5 w-5 text-gold" />}
            label="Last 7 days"
            value={money(rev?.week ?? 0)}
            delta={delta(rev?.week, rev?.prevWeek)}
          />
          <Stat
            icon={<DollarSign className="h-5 w-5 text-gold" />}
            label="This month"
            value={money(rev?.month ?? 0)}
            delta={delta(rev?.month, rev?.prevMonth)}
          />
          <Stat
            icon={<DollarSign className="h-5 w-5 text-gold" />}
            label="All time"
            value={money(rev?.allTime ?? 0)}
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <LinkStat
            ...
            accent={c?.pending ? "warn" : undefined}
          />
          <LinkStat
            to="/admin/bookings"
            ...
            value={String(c?.awaitingPayment ?? 0)}
          />
          <LinkStat
            to="/admin/driver"
            icon={<Activity className="h-5 w-5 text-gold" />}
            label="Active trips"
            value={String(c?.activeTrips ?? 0)}
            accent={c?.activeTrips ? "live" : undefined}
          />
          <LinkStat
            to="/admin/bookings"
          ...
          <LinkStat
            ...
            value={String(c?.cancelledToday ?? 0)}
          />
          <Stat
            icon={<LifeBuoy className="h-5 w-5 text-gold" />}
            label="Unread support"
            value={String(c?.unreadSupport ?? 0)}
            accent={c?.unreadSupport ? "warn" : undefined}
          />
          <Stat
            icon={<Users className="h-5 w-5 text-gold" />}
            label="Total customers"
            value={String(c?.totalCustomers ?? 0)}
          />
        </div>
      </section>
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Pickups scheduled for today, sorted by pickup time.
            </p>
          </div>
          <Link
            to="/admin/calendar"
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-widest ${urgent ? "border-yellow-500/50 text-yellow-400" : "border-gold/30 text-gold"}`}
                    >
                ...
                <div className="flex items-center gap-2">
                  <a
                    href={`tel:${b.phone}`}
                    className="rounded-full border border-border/60 px-3 py-1 text-[11px] uppercase tracking-widest text-muted-foreground hover:border-gold/40"
                  >
                    Call
                  </a>
                  <Link
                    to="/admin/bookings"
                    search={{ q: b.reservation_number } as never}
                    className="rounded-full bg-gold-gradient px-3 py-1 text-[11px] font-semibold uppercase tracking-widest text-gold-foreground"
                  >
                    Open
                  </Link>
                </div>
...
function LinkStat(
  props: React.ComponentProps<typeof Stat> & { to: string; search?: Record<string, unknown> },
) {
