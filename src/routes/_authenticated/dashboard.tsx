...
function DashboardPage() {
  ...
  const upcoming = (bookings ?? []).filter(
    (b) => new Date(b.pickup_at) >= now && !terminal.has(b.trip_status),
  );
  const past = (bookings ?? []).filter((b) => terminal.has(b.trip_status));
  const referralLink = referral
    ? `${typeof window !== "undefined" ? window.location.origin : ""}/book?ref=${referral.referral_code}`
    : "";
    ...
    <div className="mx-auto max-w-6xl px-6 py-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-2">
          {isAdmin && (
            <Link
              to="/admin"
              className="rounded-full border border-gold/40 bg-gold/10 px-5 py-2.5 text-sm font-semibold text-gold hover:bg-gold/20"
            >
              Admin dashboard
            </Link>
          )}
          <Link
            to="/book"
            className="inline-flex items-center gap-2 rounded-full bg-gold-gradient px-5 py-2.5 text-sm font-semibold text-gold-foreground shadow-gold-glow"
          >
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={(loyalty?.completed_rides ?? 0).toString()}
        />
        <StatCard
          icon={<Sparkles className="h-5 w-5 text-gold" />}
          label="Loyalty tier"
          value={profile?.loyalty_tier ?? "standard"}
        />
        <StatCard
          icon={<Gift className="h-5 w-5 text-gold" />}
          label="Referrals"
          value={(referral?.successful_referrals ?? 0).toString()}
        />
      </div>
          ...
          <Card title="Loyalty progress" accent>
            ...
            <div className="mt-3 h-2 rounded-full bg-secondary">
              <div
                ...
                style={{ width: `${Math.min(100, ((loyalty?.completed_rides ?? 0) / 20) * 100)}%` }}
              />
            </div>
            <div className="mt-3 text-xs text-muted-foreground">
              {(loyalty?.completed_rides ?? 0) < 5 &&
                `${5 - (loyalty?.completed_rides ?? 0)} more rides until 5% off`}
              {(loyalty?.completed_rides ?? 0) >= 5 &&
                (loyalty?.completed_rides ?? 0) < 10 &&
                `${10 - (loyalty?.completed_rides ?? 0)} more until 10% off`}
              {(loyalty?.completed_rides ?? 0) >= 10 &&
                (loyalty?.completed_rides ?? 0) < 20 &&
                `${20 - (loyalty?.completed_rides ?? 0)} more until VIP`}
...
function Card({
  ...
  return (
    <div
      className={`rounded-3xl border ${accent ? "border-gold/30 bg-card" : "border-border/60 bg-card/60"} p-6`}
    >
  ...
  return (
    <div className="rounded-2xl border border-dashed border-border/70 bg-secondary/30 p-8 text-center">
      ...
      {cta && (
        <Link
          to="/book"
          className="mt-4 inline-flex items-center gap-2 rounded-full bg-gold-gradient px-5 py-2 text-xs font-semibold text-gold-foreground"
        >
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-secondary/30 p-4">
      ...
      {b.trip_status === "awaiting_payment" && (
        <PayNow bookingId={b.id} deadlineAt={b.payment_deadline_at} />
      )}
      ...
      {open && (
        <div className="mt-3 space-y-2 text-xs">
          ...
          <ReceiptRow
            ...
            value={actMiles ? `${actMiles.toFixed(1)} mi` : "—"}
            highlight
          />
          ...
          {balance > 0 && (
            <ReceiptRow label="Remaining balance" value={`$${balance.toFixed(2)}`} highlight />
          )}
          ...
          {preview ? (
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                ...
                <div className="font-semibold">{preview.distanceMiles} mi</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="font-semibold text-gold">${preview.total.toFixed(2)}</div>
              </div>
            </div>
          ) : (
            <div className="text-muted-foreground">
              Pick both addresses to preview the new fare.
            </div>
          )}
        </div>
