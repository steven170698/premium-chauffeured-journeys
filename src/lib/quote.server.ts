...
export async function computeQuoteInternal(input: {
  ...
  if (!route || route.distanceMeters == null || !route.duration) {
    throw new Error("No driving route found between those addresses.");
  }

  // Google returns distance in meters and duration as a string like "1234s".
  const distanceMiles = route.distanceMeters / 1609.344;
  const durationMinutes = Number(String(route.duration).replace("s", "")) / 60;

  // Tolls are reconciled from actuals at trip end.
